...
class StateMachine {
  ...
  trigger(trig) {
    ...
    if (!dest) {
      ...
      console.log(`ERROR: No destination defined from '${this._state}' -> '${trig} transition'`);
    }

    if (this._state === dest) {
      console.log(`WARN: '${trig}' will not cause any transition as already in '${this._state}'`);
      return;
    }
