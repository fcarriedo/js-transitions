...
class StateMachine {
  ...
  trigger(trig) {
    ...
    if (!transition) {
      console.error(`ERROR: No transition found from '${this._state}' with trigger [${trig}]`);
      return;
    }
    ...
    if (!dest) {
      ...
      console.error(`ERROR: No destination defined from '${this._state}' -> [${trig}] transition'`);
    }

    if (this._state === dest) {
      console.warn(`WARN: [${trig}] will not cause any transition as already in '${this._state}'`);
      return;
    }
    ...
    console.log(`[${trig}] Successfully transitioned from '${curState["name"]}' to '${newState["name"]}'`);
