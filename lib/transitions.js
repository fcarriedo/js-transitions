...
class StateMachine {
  ...
  trigger(trig) {
    ...
    if (!dest) {
      ...
      console.log(`ERROR: No destination defined from s'${this._state}' -> t'${trig} transition'`);
    }

    if (this._state === dest) {
      console.log(`WARN: t'${trig}' will not cause any transition as already in s'${this._state}'`);
      return;
    }
